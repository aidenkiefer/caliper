...
**Current phase:** Core **Sprints 1–9** ✅, **Sprint 10 (Polymarket V1)** ✅ (**v2.0.0**), **Sprint 11 (unified pipeline)** ✅ (**v2.1.0**), **Sprint 12 (feature layer)** ✅ (**v2.2.0**), **Sprint 13 (simulation + evaluation)** ✅ (**v2.3.0**), **Sprint 14 (BTC probability model)** ✅ (**v2.4.0**; spec **AC-9** tests deferred), **Sprint 15 (regime + allocation)** ✅ (**v2.5.0**), **Sprint 16 (ranking + fleet)** ✅ (**v2.6.0**; ranking/fleet API still mock-backed — see **[docs/api-contracts.md](docs/api-contracts.md)**), **Sprint 17 (reward density + wallet intelligence + signal aggregation)** ✅ (**v2.7.0**). Before funded Polymarket sessions, complete operational setup (wallet, env, migration, dry-run)—see **[docs/POLYMARKET-QUICKSTART.md](docs/POLYMARKET-QUICKSTART.md)**. Milestone table: **[docs/plans/PROGRESS.md](docs/plans/PROGRESS.md)**.
...
**Sprint 17:** ✅ Complete (reward density + wallet intelligence + signal aggregation — `services/reward_density/`, `services/wallet_intelligence/`, `services/signal_aggregation/`, `services/fleet/lifecycle.py`, migration `014_*`, `/v1/reward-density/*`, `/v1/wallet-intelligence/*`, `/v1/signal-aggregation/*`, `/v1/lifecycle/*`)
...
- **`services/`** — Python services: **data** (migrations, including `pm.*` through `014`), **features** (incl. Polymarket sources / feature store), **backtest** (equity), **simulation** (CLOB replay / offline evaluation path), **evaluation** (metrics, baselines, reports), **portfolio** (allocator), **regime** (Sprint 15), **allocation** (Sprint 15), **ranking** (Sprint 16; includes `reward_density_score` 5th weight term from Sprint 17), **fleet** (Sprint 16 paper orchestrator + store; Sprint 17 adds `lifecycle.py`), **reward_density** (Sprint 17 — on-chain HHI, incentive model, risk scorer, density analyzer), **wallet_intelligence** (Sprint 17 — profiling, clustering, smart-money signals), **signal_aggregation** (Sprint 17 — z-score weighted aggregator with weight learning), **execution**, **risk**, **ml** (incl. **`ml/probability_model/`** for Sprint 14), **api** (FastAPI: `/v1/polymarket/*`, `/v1/simulation/*`, `/v1/evaluation/*`, `/v1/probability/*`, `/v1/regime/*`, `/v1/allocation/*`, `/v1/ranking/*`, `/v1/fleet/*`, `/v1/reward-density/*`, `/v1/wallet-intelligence/*`, `/v1/signal-aggregation/*`, `/v1/lifecycle/*`, features router), and **`polymarket`** (Sprint 10 — optional Typer CLI `polymarket-session`, CLOB/Gamma/Binance adapters, session orchestration).
