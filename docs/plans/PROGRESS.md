...
| **v2.7.0** | **Sprint 17: Reward density + wallet intelligence + signal aggregation** | Done | 2026-04-12 | 0 | `docs/plans/specs/sprint-17-reward-density-wallet-spec.md` | `docs/plans/summaries/SPRINT-17-REWARD-DENSITY-WALLET-INTELLIGENCE.md` — On-chain maker HHI (`CompetitionEstimator`), incentive model (`IncentiveEstimator`, post-Mar-30 fee formula), `RiskScorer`, `RewardDensityAnalyzer`; wallet profiling + KMeans clustering (k=4) + `WalletSignalExtractor`; `SignalAggregator` (z-score + weight learning); `LifecycleManager` (PAUSE/PROMOTE/DEMOTE/RETIRE); migration `014`; 4 API routers (`/v1/reward-density/*`, `/v1/wallet-intelligence/*`, `/v1/signal-aggregation/*`, `/v1/lifecycle/*`); `AggregatedSignal` integration into `poly_directional_v1` + `poly_hybrid_v1`; 36 tests (34 unit + 2 integration). |
