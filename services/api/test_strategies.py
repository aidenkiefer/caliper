...
class TestUpdateStrategy:
    ...
    def test_update_strategy_returns_200(self, client):
        ...
        response = client.patch("/v1/strategies/momentum_v1", json={"status": "inactive"})
        assert response.status_code == 200

    def test_update_strategy_has_message(self, client):
        """Test that update response has success message."""
        response = client.patch("/v1/strategies/momentum_v1", json={"status": "active"})
        data = response.json()
    ...
    def test_update_strategy_config(self, client):
        ...
        response = client.patch(
            "/v1/strategies/momentum_v1", json={"config": {"signal_threshold": 0.75}}
        )
        assert response.status_code == 200
    ...
    def test_update_strategy_not_found(self, client):
        ...
        response = client.patch("/v1/strategies/nonexistent_strategy", json={"status": "inactive"})
        assert response.status_code == 404

    def test_update_strategy_empty_body(self, client):
        """Test update with empty body succeeds (no-op)."""
        response = client.patch("/v1/strategies/momentum_v1", json={})
        assert response.status_code == 200
