...
class BacktestConfig(BaseModel):
    ...
    initial_capital: Decimal = Field(..., gt=0, description="Starting capital for the backtest")
    commission_per_trade: Decimal = Field(
        Decimal("1.00"), ge=0, description="Commission per trade (default: $1.00)"
    )
    ...
    start_date: Optional[datetime] = Field(
        None, description="Start date for backtest (if None, uses first bar)"
    )
    end_date: Optional[datetime] = Field(
        None, description="End date for backtest (if None, uses last bar)"
    )
...
class ParameterGrid(BaseModel):
    ...
    parameters: List[ParameterRange] = Field(
        ..., min_length=1, description="List of parameter ranges to optimize"
    )
...
class WalkForwardConfig(BaseModel):
    ...
    in_sample_days: int = Field(
        ..., gt=0, description="Number of days for in-sample (training) period"
    )
    out_of_sample_days: int = Field(
        ..., gt=0, description="Number of days for out-of-sample (test) period"
    )
    step_days: int = Field(..., gt=0, description="Number of days to step forward each iteration")
    window_type: WindowType = Field(
        WindowType.ROLLING, description="Rolling or anchored window type"
    )
    min_trades_required: int = Field(
        5, ge=0, description="Minimum trades required in in-sample for valid optimization"
    )
    objective: OptimizationObjective = Field(
        OptimizationObjective.SHARPE_RATIO, description="Optimization objective function"
    )
    parameter_grid: Optional[ParameterGrid] = Field(
        None, description="Parameter grid for optimization (None = no optimization)"
    )
...
class OptimizationResult(BaseModel):
    ...
    all_results: List[Dict[str, Any]] = Field(
        default_factory=list, description="All parameter combinations tested with results"
    )
...
class WalkForwardWindowResult(BaseModel):
    ...
    optimization_result: Optional[OptimizationResult] = Field(
        None, description="Optimization result (None if no optimization)"
    )
...
class ParameterStability(BaseModel):
    ...
    stability_score: Decimal = Field(
        ..., ge=0, le=1, description="Stability score (1.0 = perfectly stable)"
    )
...
class WalkForwardResult(BaseModel):
    ...
    aggregated_trades: List[Trade] = Field(
        default_factory=list, description="All out-of-sample trades combined"
    )
    aggregated_equity_curve: List[EquityPoint] = Field(
        default_factory=list, description="Combined out-of-sample equity curve"
    )

    # Parameter stability analysis
    parameter_stability: List[ParameterStability] = Field(
        default_factory=list, description="Parameter stability analysis"
    )
