...
class TestTradeLevelPnL:
    ...
    def test_pnl_formula_is_correct(
        self, engine: BacktestEngine, zero_slippage_config: BacktestConfig
    ):
        """
...
class TestPortfolioEquityCurve:
    ...
    def test_equity_reflects_position_value(
        ...
        strategy = BuyThenSellStrategy(quantity=Decimal("10"), buy_bar_index=0, sell_bar_index=2)
        strategy.set_total_bars(len(bars))
    ...
    def test_cash_decreases_on_buy(
        ...
        strategy = BuyThenSellStrategy(
            quantity=Decimal("10"), buy_bar_index=0, sell_bar_index=999  # Never sell
        )
        strategy.set_total_bars(len(bars))
...
class TestPnLEdgeCases:
    ...
    def test_break_even_trade(self, engine: BacktestEngine, zero_slippage_config: BacktestConfig):
    ...
    def test_high_commission_impact(self, engine: BacktestEngine, known_good_bars: List[PriceBar]):
