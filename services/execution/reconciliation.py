...
class PositionTracker:
    ...
    def open_position(
        ...
        # Update aggregate
        self._aggregate_by_symbol[symbol] = self._aggregate_by_symbol.get(symbol, Decimal(0)) + quantity
    ...
    def update_position(
        ...
        if (old_quantity > 0 and quantity_delta > 0) or (old_quantity < 0 and quantity_delta < 0):
            ...
            position.average_entry_price = total_value / total_quantity if total_quantity > 0 else Decimal(0)
    ...
    def get_positions_by_symbol(
        ...
        positions = [
            self._positions[pos_id]
            for pos_id in self._symbol_index[symbol]
        ]
    ...
    def get_total_exposure(self) -> Dict[str, Decimal]:
        ...
        for position in self.get_all_open_positions():
            if position.market_value:
                exposure[position.symbol] = exposure.get(position.symbol, Decimal(0)) + position.market_value
        return exposure
