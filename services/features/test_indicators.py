...
def test_sma():
    ...
    assert (
        sma_20.iloc[19] == df["close"].iloc[:20].mean()
    ), "SMA(20) should equal mean of first 20 values"
    assert (
        sma_20.iloc[-1] == df["close"].iloc[-20:].mean()
    ), "Last SMA should equal mean of last 20 values"
...
def test_macd():
    ...
    assert np.allclose(
        macd_result["histogram"], macd_result["macd"] - macd_result["signal"], equal_nan=True
...
def test_bollinger_bands():
    ...
    assert all(
        bb.loc[valid_mask, "upper"] >= bb.loc[valid_mask, "middle"]
    ), "Upper should be >= Middle"
    assert all(
        bb.loc[valid_mask, "middle"] >= bb.loc[valid_mask, "lower"]
