...
class ModelOutput(BaseModel):
    ...
    raw_probability: Optional[float] = Field(
        None, description="Raw model probability before thresholding"
    )
...
class ConfidenceConfig(BaseModel):
    ...
    abstain_threshold: float = Field(
        default=0.55, ge=0.0, le=1.0, description="Confidence below this triggers ABSTAIN"
    )
    low_confidence_threshold: float = Field(
        default=0.65, ge=0.0, le=1.0, description="Confidence below this is LOW_CONFIDENCE"
    )
    high_confidence_threshold: float = Field(
        default=0.85, ge=0.0, le=1.0, description="Confidence above this is HIGH_CONFIDENCE"
    )
