...
class TradeExplanation(BaseModel):
    ...
    top_features: List[FeatureContribution] = Field(
        default_factory=list, description="Top N features by absolute contribution"
    )
    model_id: str
    base_value: Optional[float] = Field(None, description="SHAP base value (expected prediction)")
