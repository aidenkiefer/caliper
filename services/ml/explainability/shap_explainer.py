...
class ShapExplainer:
    ...
    def explain(
        ...
        if len(X) == 1:
            ...
            # Average across rows
            shap_values_single = np.mean(shap_values, axis=0) if len(shap_values.shape) > 1 else shap_values
        ...
        contributions = []
        for i, (name, value, shap_val) in enumerate(zip(feature_names_list, feature_values, shap_values_single)):
