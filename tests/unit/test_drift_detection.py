...
class TestKLDivergence:
    ...
    def test_kl_divergence_asymmetry(self):
        """KL(P||Q) != KL(Q||P) in general for non-symmetric binned distributions."""
        # Use 3 bins so histograms are [0.6, 0.3, 0.1] vs [0.1, 0.2, 0.7] (not symmetric)
        p = np.array([1.0] * 60 + [2.0] * 30 + [3.0] * 10)
        q = np.array([1.0] * 10 + [2.0] * 20 + [3.0] * 70)

        kl_pq = calculate_kl_divergence(p, q, bins=3)
        kl_qp = calculate_kl_divergence(q, p, bins=3)

        # Both non-negative; KL is asymmetric so they must differ
        assert kl_pq >= 0
        assert kl_qp >= 0
...
class TestHealthScore:
    ...
    def test_healthy_model_scores_above_80(self):
        ...
        # Use recent retraining date so staleness component is high (formula uses 4 components)
        recent = datetime.utcnow() - timedelta(days=3)
        health_calculator = HealthScore(
            last_retraining_date=recent,
        )
        ...
        confidence_metric = DriftMetrics(
    ...
    def test_drifted_model_scores_below_50(self):
        ...
        health_calculator = HealthScore(last_retraining_date=datetime(2020, 1, 1))  # Very old
