...
class TestPortfolioLimits:
    ...
    def test_rejects_order_when_daily_drawdown_exceeded(self, risk_manager: RiskManager):
        ...
        assert (
            LimitType.MAX_DAILY_DRAWDOWN in violation_types
            or LimitType.KILL_SWITCH_ACTIVE in violation_types
        )
    ...
    def test_rejects_order_when_total_drawdown_exceeded(self, risk_manager: RiskManager):
        ...
        assert (
            LimitType.MAX_TOTAL_DRAWDOWN in violations or LimitType.KILL_SWITCH_ACTIVE in violations
        )
    ...
    def test_allows_sell_order_when_positions_at_max(self, risk_manager: RiskManager):
        ...
        assert result.approved is True or LimitType.MAX_OPEN_POSITIONS not in [
            v.limit_type for v in result.violations
        ]
...
class TestEdgeCases:
    ...
    def test_multiple_violations_all_reported(self, risk_manager: RiskManager):
        ...
        )  # Multiple violations: penny stock, notional, positions, capital
